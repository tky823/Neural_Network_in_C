...
int main(int argc, char **argv) {
    ...
    for(epoch = 1; epoch <= EPOCH; epoch++) {
        ...
        metrics = train_per_batch(model_parameter, LEARNING_RATE, train_data, N_train);
...
METRICS train_per_batch(MODEL_PARAMETER model_parameter, float learning_rate, DATA *train_data, int N_train) {
    ...
    metrics.E_average = metrics.E_total / (float) N_train;
    metrics.accuracy = metrics.true_count / (float) N_train;
